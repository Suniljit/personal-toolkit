...
description: >
  ...
  PR-ready plan for explicit approval, then delivers fully refactored code.
---
...
- Use Codex's normal chat updates while working: brief progress notes, clear assumptions,
  and concise questions. Do not mention unavailable Claude-only tools.
- Ask interview questions in chat. If Codex exposes a dedicated user-input tool in the
  current mode, use it; otherwise ask one concise question or tight cluster directly.
- Display plans and summaries as compact Markdown. Prefer bullets and short sections over
  wide tables or long fenced blocks so the Codex UI wraps cleanly.
- When producing the actual plan or summary, render the headings directly in the chat.
  Do not wrap the whole output in a code fence; fenced blocks below are templates only.
- When referencing local files in final answers, use Codex-style clickable links with
  absolute paths when possible. In approval plans, inline relative paths are acceptable
  for scanability.
- Keep approval gates explicit and visible. End any plan that needs approval with a
  direct line such as: "Please approve this plan before I edit files."
- Do not edit files before explicit approval of the current plan.

---

## Phase 1: Codebase Exploration

Explore before asking anything. Answer what you can from the code, while sending brief
Codex progress updates when the session expects them.

```bash
rg --files <target_dir> | sort
rg --files <target_dir> | xargs wc -l | sort -rn | head -40
```
...
Ask one question or tight cluster at a time. Always give your recommended answer with
brief rationale. Never ask what you can read.
