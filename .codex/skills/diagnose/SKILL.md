...
Use Codex's available shell and file-reading tools freely. Prefer `rg` / `rg --files` over slower search tools when available, and inspect with normal read-only commands such as `sed`, `cat`, `git log`, and `git diff`.

---

## Phase 2: Interview

Ask 1–3 targeted questions. Prioritize highest-value unknowns. Max 2 rounds.
...
Present **3–5 hypotheses**, ranked by likelihood. Use Codex-friendly Markdown sections, not fenced output templates:
...
1. Apply changes using Codex's normal editing tools. Prefer `apply_patch` for manual file edits.
...
Report with a concise Codex-friendly Markdown summary:
