...
This is four skills sharing one schema:

| Skill | Use it to... |
|---|---|
| [`wiki-ingest`](../wiki-ingest/SKILL.md) | Process new/changed files sitting in `raw/` into wiki pages |
| [`wiki-add`](../wiki-add/SKILL.md) | Feed in ad-hoc info on the spot (a paste, a note, a fact) |
| [`wiki-query`](../wiki-query/SKILL.md) | Ask the wiki a question, with citations |
| [`wiki-lint`](../wiki-lint/SKILL.md) | Health-check the wiki for contradictions, orphans, stale claims |
...
`/wiki-add` and `/wiki-query`'s "file this back" path both write into `raw/` and then run the same ingest pipeline — there's exactly one way pages get created or changed, which is what makes `/wiki-lint` and the manifest trustworthy.

## One-time setup

`wiki-common` carries its own `pyproject.toml`. Only `wiki_convert.py` needs it (non-text conversion, via [MarkItDown](https://github.com/microsoft/markitdown)) — `wiki_diff.py` is stdlib-only and runs with plain `python3`, no venv required.
...
Do this once wherever `wiki-common` lives — every project's `/wiki-ingest` shells out to this same venv via `uv run --project <path-to-wiki-common> ...`, so it's shared across every project using these skills, not reinstalled per project. If it's missing, `wiki-ingest` will tell you to run it rather than falling back to a global `pip install`.

## Setup, per project

1. Make sure these four skill folders (`wiki-ingest`, `wiki-add`, `wiki-query`, `wiki-lint`) and `wiki-common` are available to your agent harness (e.g. copied or symlinked alongside each other under `~/.claude/skills/` or the project's `.claude/skills/`). They reference each other and `wiki-common` by relative path, so `wiki-common` needs to be a sibling of the other four, not just present in this repo.
2. Drop source material into `raw/` at the project root — any mix of `.md`, `.txt`, `.pdf`, `.docx`, `.pptx`, `.xlsx`, etc.
3. Run `/wiki-ingest`.
4. `raw/`, `wiki/`, and `wiki/manifest.json` get added to `.gitignore` automatically on first run — this is local working state, not something to commit. (Karpathy's original recommends git-backing the wiki for free version history; if you want that later, just stop ignoring `wiki/`.)
...
- **"What's new" is a hash diff, not an LLM guess.** `scripts/wiki_diff.py` compares SHA-256 hashes in `manifest.json` against what's currently in `raw/` — deterministic, and it also catches *edited* files, not just new ones.
